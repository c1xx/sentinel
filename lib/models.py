...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, bitcored):
        golist = bitcored.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_bitcored(bitcored, item)
    ...
    @classmethod
    def import_gobject_from_bitcored(self, bitcored, rec):
        import decimal
        import bitcorelib
        import inflection
        ...
        # shim/bitcored conversion
        object_hex = bitcorelib.SHIM_deserialise_from_bitcored(object_hex)
        objects = bitcorelib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from bitcored...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from bitcored, with every run
        ...
        # get/create, then sync payment amounts, etc. from bitcored - Bitcored is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(bitcored)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from bitcored! %s" % e)
            govobj.vote_delete(bitcored)
            return (govobj, None)
    ...
    def vote_delete(self, bitcored):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(bitcored, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, bitcored, signal, outcome):
        import bitcorelib
        ...
        output = bitcored.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = bitcorelib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(bitcored, signal)

    def sync_network_vote(self, bitcored, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = bitcored.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = BITCORED_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 bitcore addr, non-multisig
            if not bitcorelib.is_valid_bitcore_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Bitcore address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (bitcorelib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/BitcoreDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import bitcorelib
        obj_data = bitcorelib.SHIM_serialise_for_bitcored(self.serialise())
    ...
    def prepare(self, bitcored):
        try:
            object_hash = bitcored.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = BITCORED_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not bitcorelib.is_valid_bitcore_address(addr, config.network):
    ...
    def hash(self):
        import bitcorelib
        return bitcorelib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = BITCORED_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, bitcored):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - bitcored.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, bitcored):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - bitcored.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, bitcored):
        now = int(time.time())
        return (self.created_at < (now - bitcored.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, bitcored):
        if self.is_expired(bitcored):
            return False

        return True

    def is_deletable(self, bitcored):
        if self.is_expired(bitcored):
            return True
