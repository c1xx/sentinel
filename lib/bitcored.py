...
class BitcoreDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some bitcored methods
    ...
    @classmethod
    def from_bitcore_conf(self, bitcore_dot_conf):
        from bitcore_config import BitcoreConfig
        config_text = BitcoreConfig.slurp_config_file(bitcore_dot_conf)
        creds = BitcoreConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_current_masternode_vin(self):
        from bitcorelib import parse_masternode_status_vin
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = bitcorelib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = bitcorelib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = bitcorelib.blocks_to_seconds(diff)
