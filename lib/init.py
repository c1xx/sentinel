...
def has_bitcore_conf():
    import config
    import io

    valid_bitcore_conf = False

    # ensure bitcore_conf exists & readable
    #
    # if not, print a message stating that Bitcore Core must be installed and
    # configured, including JSONRPC access in bitcore.conf
    try:
        f = io.open(config.bitcore_conf)
        valid_bitcore_conf = True
    except IOError as e:
        print(e)

    return valid_bitcore_conf
...
def main():
    ...
    if not has_bitcore_conf():
        print("BitcoreCore must be installed and configured, including JSONRPC access in bitcore.conf")
        sys.exit(1)
