...
def test_valid_bitcore_address():
    from bitcorelib import is_valid_bitcore_address

    main = valid_bitcore_address()
...
def test_invalid_bitcore_address():
    from bitcorelib import is_valid_bitcore_address

    main = invalid_bitcore_address()
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from bitcorelib import elect_mn
...
def test_hash_function():
    ...
    hex_hash = "%x" % bitcorelib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(bitcorelib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(bitcorelib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(bitcorelib.blocks_to_seconds(16616)) == 2612035
